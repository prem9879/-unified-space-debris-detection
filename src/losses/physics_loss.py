...
class PhysicsRadarLoss(nn.Module):
    ...
    """
    def __init__(self, pt: float, g: float, lamb: float, l: float, k_b: float, t: float, b: float, nf: float):
        super().__init__()
        self.pt = pt
        self.g = g
        self.lamb = lamb
    ...
    def forward(self, snr_pred: torch.Tensor, sigma_rcs: torch.Tensor, r: torch.Tensor) -> torch.Tensor:
        """
        Args:
